...
const ButtonLeft = ({ onClick, className }) => (
...
const ButtonRight = ({ onClick, className }) => (
  <button onClick={onClick} className={className}>
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
...
export default function Sponsorship() {
	const sponsorshipContainer = useRef(null);
	const displayItem1 = useRef(null);
	const displayItem2 = useRef(null);
	const displayItem3 = useRef(null);
		...
		registerScrollTrigger(() => {
			...
			const tl = gsap.timeline({
				scrollTrigger: {
					...
					snap: {
						snapTo: 1 / (displayItems.length - 1),
						duration: 0.5,
						ease: "power2.inOut"
					}
				}
	...
	function handleLeft() {
		if (currentReach > 0) {
			...
			if (window.innerWidth < 1024 && scrollContainer.current) {
				...
				gsap.to([displayItem1.current, displayItem2.current, displayItem3.current], {
					x: `+=${100}%`,
					duration: 1,
					ease: "power3.out",
				});
	...
	return (
		<section ref={sponsorshipContainer} id="sponsorship" className="relative py-[80px] lg:py-[160px] px-6 bg-black text-white">
						...
						<div className="grid grid-flow-col items-center auto-cols-[100%] overflow-hidden">
							<div ref={displayItem1} className="display-item w-full px-5 py-6 lg:pl-0 lg:py-0 lg:pr-24">
								...
								<ul className="list-disc list-inside text-sm lg:text-base text-white/75 grid gap-4 z-30 relative">
									<li>Continuous sponsor visibility via on-stage LED walls and outdoor digital screens throughout our 12-hour celebration.</li>
									<li>Reach over 60,000 engaged attendees and 100,000+ urban passersby at Sankofa Square, maximizing brand exposure.</li>
									<li>Sponsor messages are also integrated into our livestream broadcasts, expanding your reach to a vibrant digital audience</li>
								</ul>
							</div>
							...
							<div ref={displayItem2} className="display-item w-full px-5 py-6 lg:pl-0 lg:py-0 lg:pr-24">
								...
								<ul className="list-disc list-inside text-white/75 grid gap-4 z-30 relative">
									<li>From branded digital content to immersive festival activations, our team works with sponsors to design memorable brand experiences.</li>
									<li>Options include branded zones, sampling booths, interactive games, and exclusive contests—customized to your marketing goals.</li>
								</ul>
							</div>
							...
							<div ref={displayItem3} className="display-item w-full px-5 py-6 lg:pl-0 lg:py-0 lg:pr-24">
								...
								<ul className="list-disc list-inside text-white/75 grid gap-4 z-30 relative">
									<li>DESIFEST commands national and multicultural coverage on CBC, CTV, OMNI, Narcity, Todotoronto, and more.</li>
									<li>In 2025, our earned media delivered 1M+ additional impressions—at zero media spend.</li>
									<li>Sponsors feature in press releases, live interviews, on our media wall, and are celebrated as cultural champions.</li>
									<li>All partners receive a detailed post-event impact report quantifying reach and engagement.</li>
								</ul>
							</div>
						</div>
					</div>
			...
			<div className="lg:hidden absolute bottom-22 left-6 z-50 flex gap-4">
				<ButtonLeft onClick={handleLeft} className={`flex-none !size-8 ${currentReach !== 0 ? 'bg-[#FB0015]' : 'bg-[#060608]'} flex items-center justify-center text-white rounded`} />
				<ButtonRight onClick={handleRight} className={`flex-none !size-8 ${currentReach !== 2 ? 'bg-[#FB0015]' : 'bg-[#060608]'} flex items-center justify-center text-white rounded`} />	
			</div>
