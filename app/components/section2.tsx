...
export default function Section2() {
	...
	const years: number[] = Array.from({ length: 20 }, (_, i) => 2007 + i)
	...
	const currentRangeYears = years.slice(
		yearRanges[currentIndex].start,
		yearRanges[currentIndex].end
	)
	...
	const handleNext = (): void => {
		if (currentIndex < yearRanges.length - 1) setCurrentIndex(prev => prev + 1)
	}

	const handleRangeChange = (index: number): void => {
	...
	const handleTouchEnd = (): void => {
		...
		if (isLeftSwipe && currentIndex < yearRanges.length - 1) handleNext()
